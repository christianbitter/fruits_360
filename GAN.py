...
def build_and_train_models():
    ...
    inputs = tf.keras.layers.Input(shape=input_shape, name='discriminator_input')
    labels = tf.keras.layers.Input(shape=label_shape, name='class_labels')
    ...
    inputs = tf.keras.layers.Input(shape=input_shape, name='z_input')
    generator = build_generator(inputs, labels, image_size)
    generator.summary()

    # build adversarial model = generator + discriminator
    optimizer = tf.train.RMSPropOptimizer(learning_rate=lr*0.5, decay=decay*0.5)
    # freeze the weights of discriminator during adversarial training
    discriminator.trainable = False
    outputs = discriminator([generator([inputs, labels]), labels])
    adversarial = tf.keras.Model([inputs, labels], outputs, name=model_name)
